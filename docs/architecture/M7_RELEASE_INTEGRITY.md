...
The portable Linux package uses the latest stable GitHub release appcast. The legacy RC endpoint on GitHub Pages remains only for already-installed preview clients: it selects a published release containing an appcast, verifies the signature against the client key, and mirrors the unchanged appcast/signature pair so those clients can advance to stable.
...
The native packaging matrix retains outputs without writing to GitHub Releases. Stable desktop publication waits for the Store submission input and Linux packages, rebuilds and validates the desktop manifest, and uses pinned NetSparkle AppCast Generator 2.9.0 to sign each eligible Linux payload, `appcast-v2.xml`, and the manifest. The client-embedded public key must match the CI public key. The signing tool receives only a protected key-directory path; private key contents are not placed in process arguments. The asset set is first uploaded to a draft; only successful validation publishes the normal desktop release and marks it as GitHub's latest stable release. The unsigned Windows payload remains confined to the Partner Center artifact. Independent `android-v` tags build the signed APK and Android-specific manifest without republishing desktop artifacts.
