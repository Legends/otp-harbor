...
The native packaging matrix retains outputs without writing to GitHub Releases. Stable publication waits for the Store submission input, Linux packages, and the signed Android APK, then downloads only the public Linux/Android set, rebuilds and validates one aggregate manifest, and uses pinned NetSparkle AppCast Generator 2.9.0 to sign each eligible Linux payload, `appcast-v2.xml`, and the manifest. The client-embedded public key must match the CI public key. The signing tool receives only a protected key-directory path; private key contents are not placed in process arguments. The asset set is first uploaded to a draft; only successful validation publishes the normal `OTP Harbor 2.0.0` release and marks it as GitHub's latest stable release. The unsigned Windows payload remains confined to the Partner Center artifact.
